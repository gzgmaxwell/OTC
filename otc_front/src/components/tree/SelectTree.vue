...
<template>
  <div>
    ...
    <el-popover
      ...
      @hide="popoverHide"
    >
      <div class="common-tree" :style="style">
        <el-tree
          ref="tree"
          :data="data"
          :props="defaultProps"
...
export default {
  ...
  data() {
    return {
      ...
      selectedData: [], // 选中的节点
      style:
        "width: " + this.popoverWidth + "%;" + "height:" + this.height + "px;",
  ...
  mounted() {
    vm = this;
    this.popoverWidth = this.getWidth();
    window.onresize = function() {
      vm.popoverWidth = vm.getWidth();
    };
  },
  methods: {
    getWidth() {
      ...
      var doaa = this.$refs.select.$el.clientWidth;
      return doaa;
    },
    ...
    checkSelectedNode(checkedKey) {
      this.$refs.tree.setCurrentKey(checkedKey);
    ...
    popoverHide() {
      if (this.multiple) {
        ...
        this.$emit("update:checkedKeys", this.checkedIds); //更新父组件的value 值
      } else {
        this.checkedData = this.$refs.tree.getCurrentNode();
        this.$emit("update:checkedKey", this.$refs.tree.getCurrentKey()); //更新父组件的value 值
      }
