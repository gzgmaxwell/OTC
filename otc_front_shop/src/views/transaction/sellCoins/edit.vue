        ...
        <el-row :gutter="20" type="flex" class="row-bg" justify="center">
          <el-col :span="20">
            <el-form-item label="拆分方式" prop="cfcf">
              <el-radio disabled v-model="formValidate.cfcf" label="1"
                >可拆分</el-radio
              >
              <el-radio disabled v-model="formValidate.cfcf" label="2"
                >不可拆分</el-radio
              >
            </el-form-item>
          </el-col>
          ...
          <el-col :span="20">
            <el-form-item label="付款方式">
              <el-select
                multiple
                collapse-tags
                v-model="formValidate.payTypeArray"
                placeholder="请选择"
                style="width: 100%;"
              >
                <el-option
          ...
          <el-col :span="20">
            <el-form-item label="付款账户">
              <el-select
                multiple
                collapse-tags
                v-model="formValidate.payAccountIdsArray"
                placeholder="请选择"
                style="width: 100%;"
              >
                <el-option
                  v-for="(item, index) in payAccountArray"
                  :key="index"
                ...
                >
                  <span
                    v-if="item.payType == 1"
                    v-html="item.payTypeName + ' ' + item.zfbAccount"
                  ></span>
                  <span
                    v-if="item.payType == 2"
                    v-html="item.payTypeName + ' ' + item.wxAccount"
                  ></span>
                  <span
                    v-if="item.payType == 3"
                    v-html="item.bankName + ' ' + item.bankCardNumber"
                  ></span>
                </el-option>
              </el-select>
            </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        cfcf: [
          { required: true, message: "请输入拆分方式", trigger: "blur" },
          { max: 50, message: "长度最多为50", trigger: "blur" }
        ],
        zdgm: [
          { required: true, message: "请输入最低购买", trigger: "blur" },
          { max: 50, message: "长度最多为50", trigger: "blur" }
        ],
